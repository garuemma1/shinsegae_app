...
window.DailyBriefingWidget = (function () {
  ...
  function renderWidget() {
    ...
    // 특이사항 또는 당일 일지 대표 항목 추출
    const topNotable = b.importantLogs.length > 0 ? b.importantLogs[0] : (b.dayLogs.length > 0 ? b.dayLogs[0] : null);
    const topNotableAuthor = topNotable ? (topNotable.authorName || topNotable.author || '직원') : '';
    const topNotableTag = topNotable ? (topNotable.tag || topNotable.type || '일지') : '';
    const topNotableContent = topNotable ? (topNotable.content || topNotable.text || '') : '';
    ...
    let html = `
      <style>
        ...
        /* 🌟 모바일 핸드폰에서 줄바꿈 틀어짐 없이 일목요연하게 가로 스와이프되는 칩 리본 */
        ...
        .briefing-notable-banner:hover {
          transform: translateY(-1px);
        }
        .alert-notable {
          background: #fff1f2;
          border: 1.5px solid #fda4af;
          box-shadow: 0 1px 5px rgba(225,29,72,0.08);
        }
        .alert-normal-log {
          background: #f0fdf4;
      ...
      <div id="daily-briefing-box">
        ...
        <div class="briefing-bar-inner" style="border-bottom:${isExpanded ? '1.5px solid #e2e8f0' : 'none'};">
          ...
          <div class="briefing-chips-scroll">
            <div class="briefing-chip" style="background:#ecfdf5; color:#065f46; border:1px solid #a7f3d0;" onclick="event.stopPropagation(); App.navigateTo('schedule')" title="클릭 시 근무스케줄로 이동">
              📅 근무 <strong style="color:#047857;">${b.onDutyStaff.length}명</strong>
            </div>

            ${b.onLeaveStaff.length > 0 ? `
              <div class="briefing-chip" style="background:#fffbeb; color:#92400e; border:1px solid #fde68a;" onclick="event.stopPropagation(); App.navigateTo('schedule')" title="클릭 시 근무스케줄로 이동">
                🏖️ 휴가 <strong style="color:#b45309;">${b.onLeaveStaff.length}명</strong>
              </div>
            ` : ''}

            <div class="briefing-chip" style="background:#eff6ff; color:#1e40af; border:1px solid #bfdbfe;" onclick="event.stopPropagation(); App.navigateTo('worklog')" title="클릭 시 업무일지로 이동">
            ...
            <div class="briefing-chip" style="background:#fff7ed; color:#9a3412; border:1px solid #fed7aa;" onclick="event.stopPropagation(); App.navigateTo('supplies')" title="클릭 시 소모품 관리로 이동">
              📦 소모품 <strong style="color:#c2410c;">${b.daySupplies.length}건</strong>
            </div>

            <div class="briefing-chip" style="background:#f0fdf4; color:#166534; border:1px solid #bbf7d0;" onclick="event.stopPropagation(); App.navigateTo('medicine-location')" title="클릭 시 약품위치 관리로 이동">
              💊 약품위치 <strong style="color:#15803d;">${b.dayOtcMeds.length + b.dayRxMeds.length}건</strong>
            </div>

            <div class="briefing-chip" style="background:#fef2f2; color:#991b1b; border:1px solid #fecaca;" onclick="event.stopPropagation(); App.navigateTo('expiry-returns')" title="클릭 시 반품대장으로 이동">
              ⚠️ 반품 <strong style="color:#dc2626;">${b.dayReturns.length}건</strong>
            </div>

            ${b.dayNotices.length > 0 ? `
              <div class="briefing-chip" style="background:#faf5ff; color:#6b21a8; border:1px solid #e9d5ff;" onclick="event.stopPropagation(); App.navigateTo('notices')" title="클릭 시 공지사항으로 이동">
                📢 공지 <strong style="color:#7c3aed;">${b.dayNotices.length}건</strong>
              </div>
            ` : ''}

            ${b.pendingLeavesCount > 0 ? `
              <div class="briefing-chip" style="background:#fff1f2; color:#9f1239; border:1px solid #fecdd3;" onclick="event.stopPropagation(); App.navigateTo('approval')" title="클릭 시 결재함으로 이동">
                ⏳ 결재대기 <strong style="color:#be123c;">${b.pendingLeavesCount}건</strong>
              </div>
            ` : ''}
          </div>

          <!-- 3단: 🌟 업무일지 특이사항 및 인수인계 전 직원 실시간 공유 배너 -->
          ${b.importantLogs.length > 0 ? `
            <div class="briefing-notable-banner alert-notable" onclick="event.stopPropagation(); App.navigateTo('worklog')" title="클릭 시 업무일지 모듈로 이동합니다">
              <div class="notable-content-wrap">
                <span class="badge-notable-pulse">
                  <i class="fas fa-exclamation-triangle me-1"></i>특이사항 공유 (${b.importantLogs.length}건)
                </span>
                <span class="notable-text">
                  [${topNotableAuthor} · ${topNotableTag}] ${topNotableContent.replace(/\n/g, ' ')}
                </span>
              </div>
              <span class="notable-link-arrow">확인하기 <i class="fas fa-chevron-right" style="font-size:9px;"></i></span>
            </div>
          ` : (b.dayLogs.length > 0 ? `
            <div class="briefing-notable-banner alert-normal-log" onclick="event.stopPropagation(); App.navigateTo('worklog')" title="클릭 시 업무일지 모듈로 이동합니다">
              <div class="notable-content-wrap">
                <span class="badge-normal-log">
                  <i class="fas fa-clipboard-check me-1"></i>인수인계 공유 (${b.dayLogs.length}건)
                </span>
                <span class="normal-log-text">
                  [${topNotableAuthor} · ${topNotableTag}] ${topNotableContent.replace(/\n/g, ' ')}
                </span>
              </div>
              <span class="normal-link-arrow">일지 이동 <i class="fas fa-chevron-right" style="font-size:9px;"></i></span>
            </div>
          ` : `
            <div class="briefing-notable-banner alert-empty-log" onclick="event.stopPropagation(); App.navigateTo('worklog')" title="클릭 시 업무일지 작성으로 이동합니다">
              <div class="notable-content-wrap">
                <span class="badge-empty-log">
                  <i class="fas fa-info-circle me-1"></i>업무 인계
                </span>
                <span class="empty-log-text">
                  오늘 등록된 특이사항 및 인수인계 메모가 없습니다.
                </span>
              </div>
              <span class="empty-link-arrow">➕ 일지 작성 <i class="fas fa-chevron-right" style="font-size:9px;"></i></span>
            </div>
          `)}

        </div>

        <!-- 2. 펼쳤을 때 나타나는 7대 실무 모듈 브리핑 카드 뉴스 (4대 기밀/경영 탭 영구 배제) -->
        ${isExpanded ? `
          <div style="padding:16px 18px; background:#f8fafc; border-top:1px solid #e2e8f0;">
            <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(260px, 1fr)); gap:12px;">
              
              <!-- 카드 1: 월간 근무 스케줄 & 출근 현황 -->
              <div style="background:#ffffff; border:1.5px solid #e2e8f0; border-radius:14px; padding:14px; box-shadow:0 2px 6px rgba(0,0,0,0.02); display:flex; flex-direction:column; justify-content:space-between;">
                <div>
                  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <strong style="font-size:13px; color:#0f172a;"><i class="fas fa-calendar-alt text-emerald-600 me-1"></i> 1. 월간 근무 스케줄</strong>
                    <button class="btn btn-xs btn-outline-success font-bold" style="font-size:10.5px; padding:2px 6px; border-radius:6px;" onclick="App.navigateTo('schedule')">근무표 이동</button>
                  </div>
              ...
              <!-- 카드 2: 업무일지 & 특이사항 인수인계 (하이라이트 강화) -->
              <div style="background:#ffffff; border:1.5px solid #e2e8f0; border-radius:14px; padding:14px; box-shadow:0 2px 6px rgba(0,0,0,0.02); display:flex; flex-direction:column; justify-content:space-between;">
                <div>
                  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <strong style="font-size:13px; color:#0f172a;"><i class="fas fa-clipboard-list text-primary me-1"></i> 2. 업무일지 & 인수인계</strong>
                    <button class="btn btn-xs btn-outline-primary font-bold" style="font-size:10.5px; padding:2px 6px; border-radius:6px;" onclick="App.navigateTo('worklog')">업무일지 이동</button>
                  </div>
                  
                  <!-- 특이사항 강조 블록 -->
                  ${b.importantLogs.length > 0 ? `
                    <div style="background:#fff1f2; border:1.2px solid #fecdd3; border-radius:8px; padding:7px 9px; margin-bottom:8px;">
                      <div style="font-size:11.5px; color:#e11d48; font-weight:800; display:flex; align-items:center; gap:4px; margin-bottom:3px;">
                        <i class="fas fa-fire"></i> 공유 특이사항 (${b.importantLogs.length}건)
                      </div>
                      ${b.importantLogs.slice(0, 2).map(l => `
                        <div style="font-size:11.5px; color:#9f1239; line-height:1.4; margin-top:2px;">
                          • <strong>${l.authorName || '직원'}</strong> [${l.tag || '특이'}]: ${(l.content || '').slice(0, 28)}${(l.content || '').length > 28 ? '...' : ''}
                        </div>
                      `).join('')}
                    </div>
                  ` : ''}
                  ...
                  <div style="font-size:12px; color:#334155; line-height:1.5;">
                    <div>▪️ 당일 등록 일지: <strong>${b.dayLogs.length}건</strong></div>
                    <div style="margin-top:5px; max-height:65px; overflow-y:auto; background:#f8fafc; border:1px solid #e2e8f0; padding:6px 8px; border-radius:8px; font-size:11.5px;">
                      ${b.dayLogs.length > 0 ? b.dayLogs.slice(0, 3).map(l => `<div>• <strong>${l.authorName || '직원'}</strong> <span style="color:#64748b; font-size:10px;">(${l.tag || '메모'})</span>: ${(l.content || '').slice(0, 22)}${(l.content || '').length > 22 ? '...' : ''}</div>`).join('') : '<span class="text-muted">당일 등록된 새 일지가 없습니다.</span>'}
                    </div>
                  </div>
                </div>
                <div style="margin-top:8px;">
                  <button type="button" class="btn btn-sm btn-outline-primary w-100 font-bold" style="font-size:11px; padding:4px 8px; border-radius:8px;" onclick="App.navigateTo('worklog')">
                    ➕ 업무일지 / 특이사항 등록하기
                  </button>
                </div>
              </div>
              ...
              <div style="background:#ffffff; border:1.5px solid #e2e8f0; border-radius:14px; padding:14px; box-shadow:0 2px 6px rgba(0,0,0,0.02); display:flex; flex-direction:column; justify-content:space-between;">
                <div>
                  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <strong style="font-size:13px; color:#0f172a;"><i class="fas fa-map-marker-alt text-teal-600 me-1"></i> 3. 약품 위치 관리 (일반/전문)</strong>
                    <div class="d-flex gap-1">
                      <button class="btn btn-xs btn-outline-success font-bold" style="font-size:10px; padding:2px 5px; border-radius:5px;" onclick="App.navigateTo('medicine-location')">일반약</button>
                      <button class="btn btn-xs btn-outline-info font-bold" style="font-size:10px; padding:2px 5px; border-radius:5px;" onclick="App.navigateTo('rx-medicine-location')">전문약</button>
                    </div>
                  </div>
                  <div style="font-size:12px; color:#334155; line-height:1.6;">
                    <div>▪️ <strong>일반의약품 변동:</strong> ${b.dayOtcMeds.length > 0 ? `<strong class="text-success">${b.dayOtcMeds.length}건 등록/수정</strong>` : '오늘 변동 없음'}</div>
                    <div style="margin-top:2px;">▪️ <strong>조제 전문약 변동:</strong> ${b.dayRxMeds.length > 0 ? `<strong class="text-primary">${b.dayRxMeds.length}건 등록/수정</strong>` : '오늘 변동 없음'}</div>
                    ${(b.dayOtcMeds.length + b.dayRxMeds.length > 0) ? `
                      <div style="font-size:11px; color:#0f766e; margin-top:4px; background:#f0fdf4; padding:3px 6px; border-radius:6px;">
                        최근: ${[...b.dayOtcMeds, ...b.dayRxMeds].slice(0, 2).map(m => m.name).join(', ')}
                      </div>
                    ` : '<div style="font-size:11px; color:#64748b; margin-top:3px;">약품 진열 및 선반 위치 변동이 없습니다.</div>'}
                  </div>
                </div>
              </div>
              ...
              <div style="background:#ffffff; border:1.5px solid #e2e8f0; border-radius:14px; padding:14px; box-shadow:0 2px 6px rgba(0,0,0,0.02); display:flex; flex-direction:column; justify-content:space-between;">
                <div>
                  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <strong style="font-size:13px; color:#0f172a;"><i class="fas fa-boxes text-amber-500 me-1"></i> 4. 소모품 & 반품 대장</strong>
                    <div class="d-flex gap-1">
                      <button class="btn btn-xs btn-outline-secondary font-bold" style="font-size:10px; padding:2px 5px; border-radius:5px;" onclick="App.navigateTo('supplies')">소모품</button>
                      <button class="btn btn-xs btn-outline-danger font-bold" style="font-size:10px; padding:2px 5px; border-radius:5px;" onclick="App.navigateTo('expiry-returns')">반품</button>
                    </div>
                  </div>
                  <div style="font-size:12px; color:#334155; line-height:1.6;">
                    <div>▪️ <strong>당일 소모품 신청:</strong> <strong>${b.daySupplies.length}건</strong> (대기 총 ${b.pendingSuppliesCount}건)</div>
                    <div style="margin-top:2px;">▪️ <strong>유효기간 반품/폐기:</strong> <strong>${b.dayReturns.length}건 등록</strong></div>
              ...
              <div style="background:#ffffff; border:1.5px solid #e2e8f0; border-radius:14px; padding:14px; box-shadow:0 2px 6px rgba(0,0,0,0.02); display:flex; flex-direction:column; justify-content:space-between;">
                <div>
                  <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <strong style="font-size:13px; color:#0f172a;"><i class="fas fa-bullhorn text-rose-500 me-1"></i> 5. 공지사항 & 알림</strong>
                    <button class="btn btn-xs btn-outline-primary font-bold" style="font-size:10.5px; padding:2px 6px; border-radius:6px;" onclick="App.navigateTo('notices')">공지 바로가기</button>
                  </div>
                  <div style="font-size:12px; color:#334155; line-height:1.6;">
                    <div>▪️ 당일 신규 공지: <strong>${b.dayNotices.length}건</strong></div>
                    ${b.dayNotices.length > 0 ? `
                      <div style="font-size:11.5px; color:#1e40af; margin-top:3px; background:#eff6ff; padding:4px 8px; border-radius:6px;">
                        📌 ${b.dayNotices[0].title || '신규 전달사항'}
                      </div>
                    ` : '<div style="font-size:11.5px; color:#64748b; margin-top:3px;">새로 등록된 공지사항이 없습니다.</div>'}
                    ${b.pendingLeavesCount > 0 ? `<div style="margin-top:4px; font-size:11.5px; color:#dc2626;">• 결재 대기 건수: <strong>${b.pendingLeavesCount}건</strong></div>` : ''}
                  </div>
                </div>
              </div>
