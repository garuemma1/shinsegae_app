...
window.EmergencyContactsModule = (function () {
  ...
  function render(containerId) {
    ...
    const employees = (window.SheetsSync.getEmployees() || []).filter(e => e && e.role !== '예비인력');
    ...
    const html = `
      ...
      ${isSearching ? renderGlobalSearchResults(searchQuery, employees, pharmaData, equipmentData, facilitiesData, isDirector) : `
        <!-- [2] 4대 핵심 카테고리 탭 (모바일 2x2 좌우 2개씩 / PC 4열 콤팩트 그리드) -->
        <div class="mb-4" style="display:grid; grid-template-columns:repeat(auto-fit, minmax(135px, 1fr)); gap:10px;">
          ...
          <div onclick="EmergencyContactsModule.setActiveTab('family')" 
               style="cursor:pointer; border-radius:16px; padding:13px 15px; transition:all 0.2s; background:#f0fdf4; border:2px solid ${activeTab === 'family' ? '#16a34a' : '#bbf7d0'}; box-shadow:${activeTab === 'family' ? '0 6px 16px rgba(22,163,74,0.15)' : 'none'}; transform:${activeTab === 'family' ? 'translateY(-2px)' : 'none'}; display:flex; flex-direction:column; justify-content:space-between;">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <span style="font-size:13px; font-weight:800; color:#15803d; letter-spacing:-0.3px;">1. 내부 인력</span>
              <div style="width:28px;height:28px;border-radius:8px;background:#dcfce7;color:#16a34a;display:flex;align-items:center;justify-content:center;font-size:12px;"><i class="fas fa-users"></i></div>
            </div>
            <div style="font-size:20px;font-weight:800;color:#15803d;font-family:'Outfit',sans-serif;line-height:1.2;">${employees.length}<span style="font-size:12px; font-weight:600;"> 명</span></div>
            <div style="font-size:11px;color:#059669;font-weight:600;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">약국 패밀리 명부</div>
          </div>

          <!-- 2. 의약품 공급 카드 -->
          <div onclick="EmergencyContactsModule.setActiveTab('pharma')" 
               style="cursor:pointer; border-radius:16px; padding:13px 15px; transition:all 0.2s; background:#eff6ff; border:2px solid ${activeTab === 'pharma' ? '#2563eb' : '#bfdbfe'}; box-shadow:${activeTab === 'pharma' ? '0 6px 16px rgba(37,99,235,0.15)' : 'none'}; transform:${activeTab === 'pharma' ? 'translateY(-2px)' : 'none'}; display:flex; flex-direction:column; justify-content:space-between;">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <span style="font-size:13px; font-weight:800; color:#1e40af; letter-spacing:-0.3px;">2. 의약품 공급</span>
              <div style="width:28px;height:28px;border-radius:8px;background:#dbeafe;color:#2563eb;display:flex;align-items:center;justify-content:center;font-size:12px;"><i class="fas fa-pills"></i></div>
            </div>
            <div style="font-size:20px;font-weight:800;color:#1d4ed8;font-family:'Outfit',sans-serif;line-height:1.2;">${pharmaData.length}<span style="font-size:12px; font-weight:600;"> 개사</span></div>
            <div style="font-size:11px;color:#2563eb;font-weight:600;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">도매상 / 직거래</div>
          </div>

          <!-- 3. 전산 및 장비 카드 -->
          <div onclick="EmergencyContactsModule.setActiveTab('equipment')" 
               style="cursor:pointer; border-radius:16px; padding:13px 15px; transition:all 0.2s; background:#faf5ff; border:2px solid ${activeTab === 'equipment' ? '#9333ea' : '#e9d5ff'}; box-shadow:${activeTab === 'equipment' ? '0 6px 16px rgba(147,51,234,0.15)' : 'none'}; transform:${activeTab === 'equipment' ? 'translateY(-2px)' : 'none'}; display:flex; flex-direction:column; justify-content:space-between;">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <span style="font-size:13px; font-weight:800; color:#6b21a8; letter-spacing:-0.3px;">3. 전산 및 장비</span>
              <div style="width:28px;height:28px;border-radius:8px;background:#f3e8ff;color:#9333ea;display:flex;align-items:center;justify-content:center;font-size:12px;"><i class="fas fa-laptop-medical"></i></div>
            </div>
            <div style="font-size:20px;font-weight:800;color:#9333ea;font-family:'Outfit',sans-serif;line-height:1.2;">${equipmentData.length}<span style="font-size:12px; font-weight:600;"> 개처</span></div>
            <div style="font-size:11px;color:#7c3aed;font-weight:600;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">ATC / 팜IT / POS</div>
          </div>

          <!-- 4. 시설 및 소모품 카드 -->
          <div onclick="EmergencyContactsModule.setActiveTab('facilities')" 
               style="cursor:pointer; border-radius:16px; padding:13px 15px; transition:all 0.2s; background:#fffbeb; border:2px solid ${activeTab === 'facilities' ? '#d97706' : '#fde68a'}; box-shadow:${activeTab === 'facilities' ? '0 6px 16px rgba(217,119,6,0.15)' : 'none'}; transform:${activeTab === 'facilities' ? 'translateY(-2px)' : 'none'}; display:flex; flex-direction:column; justify-content:space-between;">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <span style="font-size:13px; font-weight:800; color:#92400e; letter-spacing:-0.3px;">4. 시설 및 소모품</span>
              <div style="width:28px;height:28px;border-radius:8px;background:#fef3c7;color:#d97706;display:flex;align-items:center;justify-content:center;font-size:12px;"><i class="fas fa-building"></i></div>
            </div>
            <div style="font-size:20px;font-weight:800;color:#d97706;font-family:'Outfit',sans-serif;line-height:1.2;">${facilitiesData.length}<span style="font-size:12px; font-weight:600;"> 개처</span></div>
            <div style="font-size:11px;color:#b45309;font-weight:600;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">약봉투 / 방범 / 폐기물</div>
          </div>
        </div>
        ...
        <div class="tab-content-container">
          ...
          ${activeTab === 'pharma' ? renderPharmaTab(pharmaData, isDirector, currUser) : ''}
          ${activeTab === 'equipment' ? renderEquipmentTab(equipmentData, isDirector, currUser) : ''}
          ${activeTab === 'facilities' ? renderFacilitiesTab(facilitiesData, isDirector, currUser) : ''}
        </div>
      `}

      <!-- ✏️ 협력업체 세부정보 수정 팝업 모달 -->
      <div class="modal-overlay" id="vendor-edit-modal" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.7); backdrop-filter:blur(5px); z-index:99999; justify-content:center; align-items:center;">
        <div class="modal-card shadow-lg" style="background:#fff; border-radius:24px; max-width:540px; width:92%; padding:32px; position:relative; box-shadow:0 25px 50px -12px rgba(0,0,0,0.25); max-height:90vh; overflow-y:auto;">
          <button type="button" onclick="EmergencyContactsModule.closeEditModal()" style="position:absolute; top:24px; right:24px; background:#f1f5f9; border:none; width:36px; height:36px; border-radius:50%; font-size:18px; color:#64748b; cursor:pointer; display:flex; align-items:center; justify-content:center;"><i class="fas fa-times"></i></button>
          
          <div class="d-flex align-items-center gap-3 mb-4 border-bottom pb-3">
            <div style="width:44px; height:44px; border-radius:12px; background:#eff6ff; color:#2563eb; display:flex; justify-content:center; align-items:center; font-size:18px;"><i class="fas fa-edit"></i></div>
            <div>
              <h3 style="font-size:18px; font-weight:800; margin:0; color:#0f172a;" id="vendor-edit-title">협력업체 세부정보 수정</h3>
              <p class="text-muted mb-0" style="font-size:13px; margin-top:3px;">수정된 정보는 전 직원 및 클라우드에 실시간 동기화됩니다.</p>
            </div>
          </div>

          <form onsubmit="EmergencyContactsModule.saveEditVendor(event)">
            <input type="hidden" id="edit-vendor-tab">
            <input type="hidden" id="edit-vendor-idx">

            <div class="row g-3 mb-3">
              <div class="col-md-6">
                <label class="form-label font-bold" style="font-size:13px; color:#334155;">업체 / 기관명</label>
                <input type="text" id="edit-vendor-name" class="form-control font-bold" required style="border-radius:10px; padding:10px 14px;">
              </div>
              <div class="col-md-6">
                <label class="form-label font-bold" style="font-size:13px; color:#334155;">구분 / 카테고리</label>
                <input type="text" id="edit-vendor-category" class="form-control" required style="border-radius:10px; padding:10px 14px;">
              </div>
            </div>
            ...
            <div class="row g-3 mb-3">
              <div class="col-md-6">
                ...
                <input type="text" id="edit-vendor-rep" class="form-control" style="border-radius:10px; padding:10px 14px;">
              </div>
              <div class="col-md-6">
                <label class="form-label font-bold" style="font-size:13px; color:#334155;">직통 전화번호</label>
                <input type="text" id="edit-vendor-phone" class="form-control font-bold text-primary" required style="border-radius:10px; padding:10px 14px;">
              </div>
            </div>

            <div class="mb-3">
              <label class="form-label font-bold" style="font-size:13px; color:#334155;">마감 / 운영시간</label>
              <input type="text" id="edit-vendor-cutoff" class="form-control" style="border-radius:10px; padding:10px 14px;">
            </div>

            <div class="mb-4">
              <label class="form-label font-bold" style="font-size:13px; color:#334155;">취급품목 / 주요 메모</label>
              <textarea id="edit-vendor-notes" class="form-control" rows="3" style="border-radius:10px; padding:10px 14px; font-size:13.5px;"></textarea>
            </div>

            <div class="d-flex justify-content-end gap-2">
              <button type="button" class="btn btn-light font-bold" onclick="EmergencyContactsModule.closeEditModal()" style="border-radius:10px; padding:8px 20px;">취소</button>
              <button type="submit" class="btn btn-primary font-bold" style="border-radius:10px; padding:8px 22px; background:#2563eb;"><i class="fas fa-check me-1"></i> 수정 내용 저장</button>
            </div>
          </form>
        </div>
      </div>
  ...
  /* [카테고리 2] 🏭 2. 의약품 공급 */
  function renderPharmaTab(pharmaList, isDirector, currUser) {
    return `
      <div class="card p-4 mb-4" style="border-radius:20px; border:1.5px solid #e2e8f0; background:#ffffff; box-shadow:0 4px 16px rgba(15,23,42,0.03);">
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2 border-bottom pb-3">
          <div>
        ...
        <div class="row g-3">${pharmaList.map((item, idx) => renderVendorCard('pharma', item, idx, isDirector, currUser)).join('')}</div>
      </div>
    `;
  }

  /* [카테고리 3] 💻 3. 전산 및 조제 장비 */
  function renderEquipmentTab(equipmentList, isDirector, currUser) {
    return `
      <div class="card p-4 mb-4" style="border-radius:20px; border:1.5px solid #e2e8f0; background:#ffffff; box-shadow:0 4px 16px rgba(15,23,42,0.03);">
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2 border-bottom pb-3">
          <div>
        ...
        <div class="row g-3">${equipmentList.map((item, idx) => renderVendorCard('equipment', item, idx, isDirector, currUser)).join('')}</div>
      </div>
    `;
  }

  /* [카테고리 4] 🏬 4. 소모품 및 시설 관리 */
  function renderFacilitiesTab(facilitiesList, isDirector, currUser) {
    return `
      <div class="card p-4 mb-4" style="border-radius:20px; border:1.5px solid #e2e8f0; background:#ffffff; box-shadow:0 4px 16px rgba(15,23,42,0.03);">
        <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2 border-bottom pb-3">
          <div>
        ...
        <div class="row g-3">${facilitiesList.map((item, idx) => renderVendorCard('facilities', item, idx, isDirector, currUser)).join('')}</div>
      </div>
    `;
  }
  ...
  // 🏭 협력업체 카드 (모든 직원 세부사항 100% 열람 + 수정/삭제 지원)
  function renderVendorCard(tabType, item, idx, isDirector, currUser) {
    const isPharma = tabType === 'pharma';
    const isEquip = tabType === 'equipment';
    const borderColor = isPharma ? '#0284c7' : isEquip ? '#7c3aed' : '#d97706';
        ...
        <div class="p-4 shadow-sm" style="background:#ffffff; border-radius:18px; border:1.5px solid #cbd5e1; border-left:6px solid ${borderColor}; height:100%; position:relative; display:flex; flex-direction:column; justify-content:space-between; box-shadow:0 3px 12px rgba(15,23,42,0.03);">
          ...
          ${currUser ? `
            <div style="position:absolute; top:14px; right:14px; display:flex; align-items:center; gap:6px;">
              <button type="button" class="btn btn-sm font-bold" onclick="EmergencyContactsModule.openEditModal('${tabType}', ${idx})" style="background:#eff6ff; border:1px solid #bfdbfe; color:#2563eb; font-size:11.5px; padding:3px 8px; border-radius:8px; display:inline-flex; align-items:center; gap:3px;" title="업체 세부정보 수정">
                <i class="fas fa-edit"></i> 수정
              </button>
              <button type="button" class="btn btn-sm font-bold" onclick="EmergencyContactsModule.deleteVendor('${tabType}', ${idx}, '${item.name}')" style="background:#fee2e2; border:1px solid #fecaca; color:#dc2626; font-size:11.5px; padding:3px 8px; border-radius:8px; display:inline-flex; align-items:center; gap:3px;" title="업체 삭제">
  ...
  function openEditModal(tabType, idx) {
    const data = window.SheetsSync.getEmergencyContacts() || {};
    let list = [];
    if (tabType === 'pharma') list = data.wholesalers || [];
    else if (tabType === 'equipment') list = data.equipment || data.support || [];
    else if (tabType === 'facilities') list = data.facilities || [];

    const item = list[idx];
    if (!item) return;

    document.getElementById('edit-vendor-tab').value = tabType;
    document.getElementById('edit-vendor-idx').value = idx;
    document.getElementById('edit-vendor-name').value = item.name || '';
    document.getElementById('edit-vendor-category').value = item.category || item.type || '';
    document.getElementById('edit-vendor-rep').value = item.repName || '';
    document.getElementById('edit-vendor-phone').value = item.phone || '';
    document.getElementById('edit-vendor-cutoff').value = item.cutoff || '';
    document.getElementById('edit-vendor-notes').value = item.items || item.notes || '';
    
    document.getElementById('vendor-edit-title').innerText = `[${item.name}] 세부정보 수정`;
  ...
  function saveEditVendor(e) {
    if (e) e.preventDefault();
    const tabType = document.getElementById('edit-vendor-tab').value;
    const idx = parseInt(document.getElementById('edit-vendor-idx').value);
    const name = document.getElementById('edit-vendor-name').value.trim();
    const category = document.getElementById('edit-vendor-category').value.trim();
    const repName = document.getElementById('edit-vendor-rep').value.trim();
    const phone = document.getElementById('edit-vendor-phone').value.trim();
    const cutoff = document.getElementById('edit-vendor-cutoff').value.trim();
    ...
    const data = window.SheetsSync.getEmergencyContacts() || {};
    let list = [];
    if (tabType === 'pharma') { if (!data.wholesalers) data.wholesalers = []; list = data.wholesalers; }
    else if (tabType === 'equipment') { if (!data.equipment) data.equipment = []; list = data.equipment; }
    else if (tabType === 'facilities') { if (!data.facilities) data.facilities = []; list = data.facilities; }
    ...
    if (list[idx]) {
      ...
      window.SheetsSync.saveEmergencyContacts(data);
      closeEditModal();
      alert(`🎉 [${name}] 업체의 세부정보가 성공적으로 수정 저장되었습니다!`);
      render('module-content');
    }
  }
  ...
  return { 
    render, setActiveTab, setStaffRoleFilter, toggleAddForm, 
    saveNewVendor, openEditModal, closeEditModal, saveEditVendor, 
    deleteVendor, handleSearch, applyQuickSearch, clearSearch 
  };
})();